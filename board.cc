...
Board::Board() : td {make_shared<TextDisplay>()}, level {0}, curScore {0}, highScore {0}, blockScore {0} {}
...
void Board::init() {
  ...
  for (int i = 0; i < BOARD_W; ++i) {
    newRow.emplace_back(make_shared<Cell>());
  }

  for (int grid_r = 0; grid_r < BOARD_H + RESERVED; ++grid_r) {
    theBoard.emplace_back(newRow);
  }
...
void Board::lineClear(int row) { // add lose condition, and check block type disappear
  ...
  vector<vector<shared_ptr<Cell>>>::iterator it = theBoard.begin();
